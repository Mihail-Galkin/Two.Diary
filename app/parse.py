...
def auth(login: str, password: str) -> requests.Session:
    ...
    session.post(url, data=data, proxies={os.getenv("proxy1"): os.getenv("proxy2")}, verify="43edu-ru-chain.pem")

    return session


def get_guid(session: requests.Session) -> str:
    response = session.get("https://one.43edu.ru/edv/index/participant", proxies={os.getenv("proxy1"): os.getenv("proxy2")}, verify="43edu-ru-chain.pem")
...
def get_raw_diary(session: requests.Session, guid: str, date: Union[str, d], retry=True) -> Optional[Tuple[dict, requests.Session]]:
    ...
    data = {'date': date}
    response = session.get(url, params=data, proxies={os.getenv("proxy1"): os.getenv("proxy2")}, verify="43edu-ru-chain.pem")
    json = response.json()
    if json["success"]:
        return json, session
